...
uint32 Unit::DealDamage(DamageInfo* damageInfo)
{
    ...
    if ( pVictim->GetTypeId() == TYPEID_PLAYER )
    {
        ...
        for (AuraList::const_iterator i = BlessedLife.begin(); i != BlessedLife.end(); ++i)
        {
            ...
            if ((*i)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_PALADIN && (*i)->GetSpellProto()->GetSpellIconID() == 2137)
                if ( urand(0,100) < (*i)->GetSpellProto()->GetProcChance() )
            ...
            if (damageInfo->damageType != DOT)
            {
                for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; ++i)
                {
                    ...
                    if (Spell* spell = pVictim->GetCurrentSpell(CurrentSpellTypes(i)))
                    {
                        ...
                        {
                            if (spell->m_spellInfo->GetInterruptFlags() & SPELL_INTERRUPT_FLAG_ABORT_ON_DMG) // Always interrupt, even on absorbed.
...
void Unit::CalculateSpellDamage(DamageInfo* damageInfo, float DamageMultiplier)
{
    ...
    switch (damageInfo->GetSpellProto()->GetDmgClass())
    {
...
void Unit::CalculateMeleeDamage(DamageInfo* damageInfo)
{
    ...
    switch(damageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_BLOCK:
        {
            ...
            damageInfo->blocked     = damageInfo->target->GetShieldBlockDamageValue() * damageInfo->damage / 100.0f;
...
void Unit::CalculateDamageAbsorbAndResist(Unit* pCaster, DamageInfo* damageInfo, bool canReflect)
{
    ...
    if (!vManaShield.empty())
    {
        for(AuraList::const_iterator i = vManaShield.begin(), next; i != vManaShield.end() && RemainingDamage > 0; ++i)
        {
            ...
            if (float manaMultiplier = (*i)->GetSpellEffect()->EffectMultipleValue)
            {
    ...
    if (pCaster != this)
    {
        ...
        for(AuraList::const_iterator i = vSplitDamagePct.begin(), next; i != vSplitDamagePct.end() && RemainingDamage >= 0; i = next)
        {
            ...
            pCaster->SendSpellNonMeleeDamageLog(caster, (*i)->GetSpellProto()->Id, splitdamageInfo.damage, damageInfo->SchoolMask(), splitdamageInfo.GetAbsorb(), 0, false, 0, false);
...
float Unit::CalculateLevelPenalty(SpellEntry const* spellProto) const
{
    ...
    if (spellProto->GetMaxLevel() <= 0)
        return 1.0f;
    //if caster level is lower that max caster level
    if (getLevel() < spellProto->GetMaxLevel())
        return 1.0f;

    float LvlPenalty = 0.0f;

    LvlPenalty = (22.0f + float (spellProto->GetMaxLevel()) - float (getLevel())) / 20.0f;
...
int32 Unit::GetTotalAuraModifierByMiscMask(AuraType auratype, uint32 misc_mask) const
{
    ...
    for(AuraList::const_iterator i = mTotalAuraList.begin();i != mTotalAuraList.end(); ++i)
    {
        ...
        if (auratype == SPELL_AURA_MOD_DAMAGE_DONE)
        {
            ...
            if (eqItems->EquippedItemClass != -1 ||               // -1 == any item class (not wand then)
                eqItems->EquippedItemInventoryTypeMask != 0)      //  0 == any inventory type (not wand then)
            {
                continue;
            }
        }
...
void Unit::RemoveAuraHolderDueToSpellByDispel(uint32 spellId, uint32 stackAmount, ObjectGuid casterGuid, Unit *dispeller)
{
    ...
    // Custom dispel case
    // Unstable Affliction
    if (spellEntry->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && spellEntry->GetSpellFamilyFlags().test<CF_WARLOCK_UNSTABLE_AFFLICTION>())
    {
    ...
    // Lifebloom
    else if (spellEntry->GetSpellFamilyName() == SPELLFAMILY_DRUID && spellEntry->GetSpellFamilyFlags().test<CF_DRUID_LIFEBLOOM>())
    {
    ...
    // Flame Shock
    else if (spellEntry->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && spellEntry->GetSpellFamilyFlags().test<CF_SHAMAN_FLAME_SHOCK>())
    {
        Unit* caster = NULL;
...
void Unit::RemoveAurasDueToSpellBySteal(uint32 spellId, ObjectGuid casterGuid, Unit *stealer)
{
    ...
    // Lifebloom
    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID && spellProto->GetSpellFamilyFlags().test<CF_DRUID_LIFEBLOOM>())
    {
        Aura* dotAura = GetAura<SPELL_AURA_DUMMY, SPELLFAMILY_DRUID, CF_DRUID_LIFEBLOOM>(casterGuid);
        if (dotAura)
        {
            ...
            if (Unit* caster = dotAura->GetCaster())
            {
                int32 returnmana = (spellProto->GetManaCostPercentage() * caster->GetCreateMana() / 100) * dotAura->GetStackAmount() / 2;
                caster->CastCustomSpell(caster, 64372, &returnmana, NULL, NULL, true, NULL, dotAura, casterGuid);
            }
        }
    }
...
void Unit::RemoveAurasWithDispelType(DispelType type, ObjectGuid casterGuid)
{
    ...
    SpellIdSet spellsToRemove;
    {
        ...
        for (SpellAuraHolderMap::const_iterator iter = holdersMap.begin(); iter != holdersMap.end(); ++iter)
        {
            ...
            if (((1 << iter->second->GetSpellProto()->GetDispel()) & dispelMask) && (!casterGuid || casterGuid == iter->second->GetCasterGuid()))
...
Aura* Unit::GetAuraByEffectMask(AuraType type, SpellFamily family, ClassFamilyMask const& classMask, ObjectGuid casterGuid)
{
    ...
    for (AuraList::iterator i = auras.begin(); i != auras.end(); ++i)
    {
        ...
        if ((*i)->GetAuraSpellClassMask() == classMask &&
            (family <= SPELLFAMILY_PET &&  (*i)->GetSpellProto()->GetSpellFamilyName() == family) &&
            (!casterGuid || (*i)->GetCasterGuid() == casterGuid))
            return (*i)();
    }
        ...
        if (calculatedBonus == 0)
        {
            ...
            if ((*i)->GetSpellProto()->Id == 16246)
            {
                ...
                for (AuraList::const_iterator itr = aurasCrit.begin(); itr != aurasCrit.end(); itr++)
                {
                    ...
                    {
                        calculatedBonus = (*itr)->GetSpellProto()->GetSpellEffect(EFFECT_INDEX_1)->CalculateSimpleValue();
                        break;
                    }
                }
            }
                ...
                switch(spellProto->GetSpellFamilyName())
                {
                    ...
                    case SPELLFAMILY_DRUID:
                        ...
                        if (pVictim->HasAura(770) || pVictim->HasAura(16857))
                        {
                            ...
                            {
                                if((*iter)->GetEffIndex() == 0 && (*iter)->GetSpellProto()->GetSpellIconID() == 109 && (*iter)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DRUID)
                                {
                                    crit_chance += (*iter)->GetModifier()->m_amount;
                                    break;
                                }
...
uint32 Unit::SpellHealingBonusDone(Unit *pVictim, SpellEntry const *spellProto, int32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    SpellClassOptionsEntry const* classOptions = spellProto->GetSpellClassOptions();
    if (classOptions && classOptions->GetSpellFamilyName() == SPELLFAMILY_DRUID && spellProto->GetSpellFamilyFlags().test<CF_DRUID_NOURISH>())
    {
        int ownHotCount = 0;                        // counted HoT types amount, not stacks
        Unit::AuraList const& RejorRegr = pVictim->GetAurasByType(SPELL_AURA_PERIODIC_HEAL);
        for(Unit::AuraList::const_iterator i = RejorRegr.begin(); i != RejorRegr.end(); ++i)
    ...
    if (!isWeaponDamageBasedSpell)
    {
        ...
        for(AuraList::const_iterator i = mModDamageDone.begin(); i != mModDamageDone.end(); ++i)
        {
            ...
            if ((((*i)->GetModifier()->m_miscvalue & damageInfo->SchoolMask()) ||                          // schoolmask has to fit with the intrinsic spell school
                ...
                ((holder->GetSpellProto()->GetEquippedItemClass() == -1) ||                     // general, weapon independent
...
void Unit::MeleeDamageBonusTaken(DamageInfo* damageInfo, uint32 stack)
{
    ...
    SpellClassOptionsEntry const* classOptions = damageInfo->GetSpellProto() ? damageInfo->GetSpellProto()->GetSpellClassOptions() : NULL;
    if (damageInfo->GetSpellProto() && damageInfo->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DRUID && damageInfo->GetSpellProto()->GetSpellFamilyFlags().test<CF_DRUID_MAUL, CF_DRUID_SHRED>())
...
*/
        ...
        for (SpellAuraHolderMap::const_iterator itr = holderMap.begin(); itr != holderMap.end(); ++itr)
        {
            ...
            if (itr->second->GetCaster() == pTarget)
               if (SpellEntry const* spellInfo = itr->second->GetSpellProto())
                  if (procSpell && spellInfo->GetSpellFamilyName() == SPELLFAMILY_MAGE && spellInfo->GetSpellFamilyFlags().test<CF_MAGE_FROST_NOVA>()
                     && procSpell->GetSpellFamilyName() == SPELLFAMILY_MAGE && procSpell->GetSpellFamilyFlags().test<CF_MAGE_FROST_NOVA>())
                        continue;
    ...
    for (ProcTriggeredList::const_iterator itr = procTriggered.begin(); itr != procTriggered.end(); ++itr)
    {
        ...
        for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
        {
            ...
            SpellEffectEntry const* spellEffect = triggeredByAura->GetSpellEffect();
            if (!spellEffect)
                continue;
            ...
            SpellAuraProcResult procResult = (*this.*AuraProcHandler[spellEffect->EffectApplyAuraName])(pTarget, damageInfo, triggeredByAura, procSpell, procFlag, procExtra, cooldown);
...

    {
        ...
        for (SpellAuraHolderMap::const_iterator iter = holders.begin(); iter != holders.end(); ++iter)
        {
            ...
            if (iter->second->HasMechanicMask(mechMask))
            {
                ...
                for (int32 i = 0; i < MAX_EFFECT_INDEX; i++)
                {
                    SpellEffectEntry const* spellEffect = iter->second->GetSpellProto()->GetSpellEffect(SpellEffectIndex(i));
                    if (!spellEffect)
                        continue;

                    uint8 mechanic  = spellEffect->EffectMechanic ?
                                      spellEffect->EffectMechanic :
                                      iter->second->GetSpellProto()->GetMechanic();
...
void Unit::RemoveAurasBySpellMechanic(uint32 mechMask)
{
    ...
    for(Unit::SpellAuraHolderMap::iterator iter = holders.begin(); iter != holders.end();)
    {
        if (!iter->second || iter->second->IsDeleted() || !iter->second->IsPositive())
            ++iter;
        else if (iter->second->GetSpellProto()->GetMechanic() & mechMask)
        {
            ...
            for (SpellClickInfoMap::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
            {
                if (itr->second.IsFitToRequirements((Player*)this, (Creature*)vehicleBase))
                {
                    ...
                    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
                    {
                        ...
                        {
                            if (spellInfo->GetSpellEffect(SpellEffectIndex(i))->EffectApplyAuraName == SPELL_AURA_CONTROL_VEHICLE)
                                b_controlAura = true;
                        }
                    }
...
bool Unit::HasMorePoweredBuff(uint32 spellId)
{
    ...
    for (uint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        AuraType auraType = AuraType(spellEffect->EffectApplyAuraName);
        ...
        for (SpellAuraHolderMap::const_iterator itr = holders.begin(); itr != holders.end(); ++itr)
        {
            ...
            {
                SpellEffectEntry const* foundSpellEffect = foundSpellInfo->GetSpellEffect(SpellEffectIndex(j));
                if(!foundSpellEffect)
                    continue;

                if ( foundSpellEffect->Effect != SPELL_EFFECT_APPLY_AURA  &&
                     foundSpellEffect->Effect != SPELL_EFFECT_APPLY_AREA_AURA_PARTY &&
                     foundSpellEffect->Effect != SPELL_EFFECT_APPLY_AREA_AURA_RAID
                    )
                    continue;

                if (foundSpellEffect->Effect != spellEffect->Effect)
                    continue;

                if (foundSpellEffect->EffectApplyAuraName != spellEffect->EffectApplyAuraName)
                    continue;

                if (foundSpellEffect->EffectMiscValue != spellEffect->EffectMiscValue)
                    continue;

                if (spellEffect->CalculateSimpleValue() < foundSpellEffect->CalculateSimpleValue())
                    return true;
                else
                    return false;
            }
...
SpellSchoolMask DamageInfo::SchoolMask() const
{
    return GetSpellProto() ?
        SpellSchoolMask(GetSpellProto()->GetSchoolMask()) :
...
void Unit::AddSpellAndCategoryCooldowns(SpellEntry const* spellInfo, uint32 itemId /*= 0*/, bool infinityCooldown  /*= false*/)
{
    ...
    if (cooldown < 0 && categorycooldown < 0)
    {
        ...
        cooldown = spellInfo->GetRecoveryTime();
        categorycooldown = spellInfo->GetCategoryRecoveryTime();
    }
