...
LanguageDesc lang_description[LANGUAGES_COUNT] =
{
    ...
    { LANG_PANDAREN_TUSHUI, 108130, SKILL_LANG_PANDAREN_ALLIANCE },
    { LANG_PANDAREN_HUOJIN, 108131, SKILL_LANG_PANDAREN_HORDE }
};
...
void ObjectMgr::ConvertCreatureAddonAuras(CreatureDataAddon* addon, char const* table, char const* guidEntryStr)
{
    ...
    for(uint32 j = 0; j < val.size(); ++j)
    {
        ...
        {
            sLog.outDetail("Creature (%s: %u) has spell %u defined in `auras` field in `%s, but spell doesn't apply an aura`.", guidEntryStr, addon->guidOrEntry, cAura, table);
        }
        ...
        {
            sLog.outDetail("Creature (%s: %u) has spell %u defined in `auras` field in `%s, but spell is no self-only spell`.", guidEntryStr, addon->guidOrEntry, cAura, table);
        }
...
std::string ObjectMgr::GeneratePetName(uint32 entry)
{
    ...
    if (list0.empty() || list1.empty())
    {
        ...
        return std::string(petname ? petname : "");
    }
    ...
    switch (m_condition)
    {
        ...
        case CONDITION_AD_COMMISSION_AURA:
        {
            ...
            for (Unit::SpellAuraHolderMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                if ((itr->second->GetSpellProto()->HasAttribute(SPELL_ATTR_CASTABLE_WHILE_MOUNTED) || itr->second->GetSpellProto()->HasAttribute(SPELL_ATTR_ABILITY)) && itr->second->GetSpellProto()->GetSpellVisual() == 3580)
                    return true;
            return false;
        }
