...
char remotes[NUM_REMOTES][MAX_REMOTE] = {
    "git@github.com:mangosR2/mangos4.git",
    "git://github.com/mangosR2/mangos4.git"        // used for fetch if present
};

char remote_branch[MAX_REMOTE] = "master";
char rev_nr_file[MAX_PATH] = "src/shared/revision_R2.h";
...
std::string generateNrHeader(char const* rev_str)
{
    ...
    newData << "#ifndef __REVISION_R2_H__" << std::endl;
    newData << "#define __REVISION_R2_H__"  << std::endl;
    newData << " #define REVISION_R2 \"" << rev_str << "\"" << std::endl;
    newData << "#endif // __REVISION_R2_H__" << std::endl;
    return newData.str();
}
...
bool amend_commit()
{
    ...
    fprintf(cmd_pipe, "[mq%d] %s", rev, head_message);
    pclose(cmd_pipe);
